...
handleLinePhase scanline' cycle = do
  ...
  when rendering $ do
    ...
    when
      (preLine && cycle >= 280 && cycle <= 304)
      copyY

    when (preLine || visibleLine) $ do
      when
        ((preFetchCycle || visibleCycle) && cycle `rem` 8 == 0)
        incrementX
...
getSpritePixel coords = do
  ...
  where
    ...
    getColor sprite@(Sprite _ (x', _y) _ _ sPattern' _) = do
...
getComposedColor (x', _y) bg sprite = do
  color <- getColor
