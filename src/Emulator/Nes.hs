...
new cart' = do
  ...
  Nes cpu' ppu' cart' mapper' <$> Controller.new
...
readMapper addr = with mapper (`Mapper.read` addr)
...
writeNametableData addr v = do
  mirror' <- with cart (readIORef . Cartridge.mirror)
...
loadScreen = with ppu (pure . screen)
